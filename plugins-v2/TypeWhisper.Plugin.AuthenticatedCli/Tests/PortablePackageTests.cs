...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new AuthenticatedCliPlugin();
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-AuthenticatedCli-"+Guid.NewGuid().ToString("N"));
