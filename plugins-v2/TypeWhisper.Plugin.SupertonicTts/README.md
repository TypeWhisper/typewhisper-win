...
The Windows settings code is prepared for joint acceptance with Gemma. The app build, rendered model card, checkbox, progress/cancel interaction, settings screenshot and audible playback still need that shared acceptance session. App builds and restarts were intentionally deferred while other worktrees use the same development output. No production installation, public catalog publication or PR push has been performed for this version.
