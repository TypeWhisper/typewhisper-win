...
public sealed partial class GeminiPlugin :
    ...
    ILlmProviderPlugin,
    ILlmRequestHedgingSupport, IApiKeyPlugin, IPluginTextSettings, IPluginSettingsActions
{
    ...
    internal delegate Task<IStreamingSession> StreamConnector(string key, string model, IReadOnlyList<string> languages,
        IReadOnlyList<string> vocabulary, GeminiTranscriptionMode mode, CancellationToken ct);
    private readonly StreamConnector _connectStream;
    ...
    /// </summary>
    public GeminiPlugin()
        : this(new HttpClient(new HttpClientHandler { AllowAutoRedirect = false }) { Timeout = TimeSpan.FromMinutes(15) })
    {
    }

    internal GeminiPlugin(HttpClient httpClient, StreamConnector? connectStream = null)
    {
        _httpClient = httpClient;
        _connectStream = connectStream ?? (async (key, model, languages, vocabulary, mode, ct) =>
            await GeminiStreamingSession.ConnectAsync(key, model, languages, vocabulary, mode, ct));
    }
    ...
    public async Task ActivateAsync(IPluginHostServices host)
    {
        ...
        var options = host.GetSetting<LlmOptions>("llmOptions") ?? new();
        _llmOptions = options with
        {
            TemperatureMode = options.TemperatureMode == "custom" ? "custom" : "providerDefault",
            Temperature = double.IsFinite(options.Temperature) ? Math.Clamp(options.Temperature, 0, 2) : 0.3
        };
    ...
    public Task<IStreamingSession> StartStreamingWithLanguageHintsAsync(
        IReadOnlyList<string> languageHints,
        CancellationToken ct) => StartStreamingWithLanguageHintsAndPromptAsync(languageHints, null, ct);
        ...
        CancellationToken ct)
    {
        ...
        if (!IsCompatibleChatModelId(modelId)) throw new ArgumentException("Invalid Gemini text model.", nameof(model));
        ...
        CancellationToken ct)
    {
        ...
        if (_llmOptions.TemperatureMode == "custom") body["temperature"] = _llmOptions.Temperature;
    ...
    private static string NormalizeModelId(string id)
    {
        ...
        if (normalized.Length > 256 || normalized.Any(char.IsControl) || normalized.Any(char.IsWhiteSpace)
            || normalized.IndexOfAny(['?', '#', '\\']) >= 0)
            throw new ArgumentException("Invalid Gemini model ID.");
    ...
    private static string? NormalizeLanguage(string? language)
    {
        ...
        return string.IsNullOrWhiteSpace(normalized)
            || normalized.Equals("auto", StringComparison.OrdinalIgnoreCase)
                ? null
                : LanguageCodes.GetValueOrDefault(normalized, normalized);
    }
    ...
    private static string? NormalizeApiKey(string? apiKey)
    {
        ...
        var key = apiKey.Trim();
        if (key.Length > 512 || key.Any(char.IsControl)) throw new ArgumentException("Invalid API key format.");
        return key;
    }
