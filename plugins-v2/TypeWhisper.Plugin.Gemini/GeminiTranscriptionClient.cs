...
internal static class GeminiTranscriptionClient
{
    ...
    internal static string ParseInteractionText(string json)
    {
        try
        {
            ...
            if (TryGetString(root, "output_text", out var outputText) && !string.IsNullOrWhiteSpace(outputText))
                return outputText.Trim();

            if ((!root.TryGetProperty("steps", out var steps) && !root.TryGetProperty("outputs", out steps))
            ...
            {
                if ((TryGetString(step, "type", out var type) && !type.Equals("model_output", StringComparison.OrdinalIgnoreCase))
        ...
        CancellationToken ct)
    {
        ...
        if (!Uri.TryCreate(uploadUrl, UriKind.Absolute, out var uploadUri) || uploadUri.Scheme != Uri.UriSchemeHttps
            || uploadUri.Host != new Uri(baseUrl).Host || !string.IsNullOrEmpty(uploadUri.UserInfo))
            throw new PluginRequestException("Gemini returned an invalid upload URL.", PluginRequestFailureKind.OutputIncomplete);
    ...
    private static GeminiUploadedFile ParseUploadedFile(string json)
    {
        try
        {
            ...
            if (!document.RootElement.TryGetProperty("file", out var file)
                ...
                || !System.Text.RegularExpressions.Regex.IsMatch(name, @"\Afiles/[A-Za-z0-9_-]+\z"))
            {
                throw new PluginRequestException(
