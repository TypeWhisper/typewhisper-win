...
All 56 plugin tests and 259 portable SDK/host tests passed. Fake HTTP/WebSocket tests cover payloads, errors, cancellation, polling timeout, multilingual routing, dictionary budgets, speaker labels, timestamps, formatted-turn deduplication, final-tail flushing and termination acknowledgement. The real package-store/runtime test covers ZIP identity/hash, installation, configuration, restart, duplicate-install rejection, uninstall/reinstall and retained credentials/settings. The ZIP contains only the provider DLL, dependency manifest and plugin manifest; no host SDK, WPF or test assemblies.
