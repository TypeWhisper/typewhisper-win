...
{
    private static readonly string Id="com.typewhisper.cohere";
    ...
    {
        var host=new Host(); using var plugin=new CoherePlugin(); await plugin.ActivateAsync(host); await Configure(plugin);
    ...
    {
        var host=new Host(); using var plugin=new CoherePlugin(); await plugin.ActivateAsync(host);
    ...
    {
        using var plugin=new CoherePlugin();await plugin.ActivateAsync(new Host());await Configure(plugin);
    ...
    {
        var root=Path.Combine(Path.GetTempPath(),"cohere-package-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);
        try
        {
            ...
            var entry=new PortableCatalogEntry{Id=Id,Name="Cohere",Version="1.1.1",MinHostVersion="1.1.2",DownloadUrl="https://fixture.invalid/plugin.zip",Sha256=Convert.ToHexString(SHA256.HashData(bytes)),Size=bytes.Length,SupportedArchitectures=[PortablePluginCatalog.Architecture]};
