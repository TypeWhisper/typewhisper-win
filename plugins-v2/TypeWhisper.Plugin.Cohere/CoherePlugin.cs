...
/// <summary>Independent portable Cohere provider, compared with Windows 4db8f6ac and macOS ac00e39e.</summary>
public sealed partial class CoherePlugin : ITranscriptionEnginePlugin, ILlmProviderPlugin, ILlmRequestHedgingSupport, IApiKeyPlugin, IPluginTextSettings
{
    private readonly ProviderConnection Connection;
    /// <summary>Creates a provider with isolated HTTP transport.</summary>
    public CoherePlugin() : this(new HttpClient(new HttpClientHandler { AllowAutoRedirect = false }) { Timeout = TimeSpan.FromMinutes(5) }) { }
    internal CoherePlugin(HttpClient http) => Connection = new(http);
    /// <inheritdoc />
    ...
    /// <inheritdoc />
    public IReadOnlyList<PluginModelInfo> TranscriptionModels { get; } = [new("cohere-transcribe-03-2026", "Cohere Transcribe")];
    /// <inheritdoc />
