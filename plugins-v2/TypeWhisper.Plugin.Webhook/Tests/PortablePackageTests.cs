...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new WebhookPlugin();
    ...
    public async Task Package_InstallsLoadsRestartsAndUninstallsIndependently()
    {
        ...
        PortablePluginStore Store() => new(Path.Combine(fixture.Root,"store"),new(1,1,2),http,_ => fixture.Host);
        ...
        await using (var runtime=new PortablePluginRuntimeRegistry(store,new(1,1,2),_ => fixture.Host))
        {
        ...
        await using (var runtime=new PortablePluginRuntimeRegistry(restart,new(1,1,2),_ => fixture.Host))
        {
        ...
        await using var package=await PortablePluginPackage.LoadAsync(restart.Resolve(entry.Id),fixture.Host,new(1,1,2));
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-Webhook-"+Guid.NewGuid().ToString("N"));
