...
public sealed partial class ClaudePlugin
{
    private const string ProfileId = "claude";
    /// <inheritdoc />
    ...
    [
        new(ProfileSelectorId, "Claude", "", ProfileId)
        { Section = PluginSettingsSection.Connection, Choices = [new(ProfileId, "Claude")] },
        ...
        new("llmTemperatureValue", L("Temperature", "Temperatur"),
            L("A value between 0 and 1 for older models. Newer models always use the provider default.", "Ein Wert zwischen 0 und 1 für ältere Modelle. Neuere Modelle verwenden immer die Anbietervorgabe."),
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (profileId != ProfileId) throw new ArgumentException("Unknown Claude configuration.");
        ...
        foreach (var (id, value) in values)
        {
            if (!fields.TryGetValue(id, out var field) || value.Length > field.MaxLength
                ...
                throw new ArgumentException("Invalid Claude setting.");
            next = id switch
            {
                ...
                _ => throw new ArgumentException("Unknown Claude setting.")
            };
        }
    ...
    private static double ParseTemperature(string value) => double.TryParse(value.Replace(',', '.'), NumberStyles.Float,
        CultureInfo.InvariantCulture, out var number) && double.IsFinite(number) && number >= 0 && number <= 1
        ? number : throw new ArgumentException("Temperature must be between 0 and 1.");
    ...
    private static async Task TryDeleteSecretAsync(IPluginHostServices host, string name)
    {
        ...
        { host.Log(PluginLogLevel.Warning, "An inactive encrypted Claude key could not be removed."); }
    }
        ...
        IReadOnlyDictionary<string, string> values, string? apiKey, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (profileId != ProfileId) throw new ArgumentException("Unknown Claude configuration.");
        if (actionId is not ("checkConnection" or "refreshModels")) throw new ArgumentException("Unknown Claude action.");
        var generation = Interlocked.Increment(ref _generation);
        var key = NormalizeKey(apiKey) ?? _apiKey;
        var models = await FetchModelsAsync(key, cancellationToken);
        cancellationToken.ThrowIfCancellationRequested();
        if (_host is null || generation != Interlocked.Read(ref _generation))
            throw new OperationCanceledException("The Claude configuration changed during model discovery.");
