...
internal sealed class OpenAiRealtimeStreamingSession : IStreamingSession
{
    ...
    public static async Task<OpenAiRealtimeStreamingSession> ConnectAsync(
        ...
        string? prompt,
        CancellationToken ct, IReadOnlyList<string>? keywords = null, string delay = LiveDelay)
    {
        var ws = CreateConfiguredWebSocket(apiKey);
        OpenAiRealtimeStreamingSession? session = null;
        try
        {
    ...
    public static async Task<PluginTranscriptionResult> TranscribeWavAsync(
        ...
        CancellationToken ct, IReadOnlyList<string>? keywords = null, string delay = LiveDelay)
    {
        await using var session = await ConnectAsync(apiKey, modelId, languageHints, prompt, ct, keywords, delay);
    ...
    internal async Task StartAsync(string modelId, IReadOnlyList<string> hints, string? prompt, CancellationToken ct,
        IReadOnlyList<string>? keywords = null, string delay = LiveDelay)
    {
    ...
    internal static string CreateSessionUpdatePayload(
        string modelId,
        IReadOnlyList<string> languageHints,
        string? prompt, IReadOnlyList<string>? keywords = null, string delay = LiveDelay)
    {
