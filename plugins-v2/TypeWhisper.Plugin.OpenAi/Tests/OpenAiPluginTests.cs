...
public partial class OpenAiPluginTests
{
    ...
    public async Task GPTTranscribeRequest_UsesOrderedPluralLanguagesAndDictionaryPrompt()
    {
        ...
        Assert.Equal(["TypeWhisper", "OpenAI"], fields.Where(field => field.Name == "keywords[]").Select(field => field.Value));
        Assert.DoesNotContain(fields, field => field.Name == "prompt");
    }
    ...
    public async Task GPTTranscribe_UsesCompressedUploadAndPreservesContextFields()
    {
        ...
        var handler = new CapturingHandler((request, body) =>
        {
            ...
            AssertMultipartToken(body, "name", "keywords[]");
    ...
    [Theory]
    [InlineData(415, "Unsupported audio format")]
    [InlineData(500, "ffprobe failed for private-provider-detail")]
    public async Task GPTTranscribe_FormatRejectionRetriesWithWavAndPreservesFields(int status, string message)
    {
    ...
    [Fact]
    public async Task WhisperTranslation_OmitsSourceLanguageAndPreservesPrompt()
    {
        string? body = null;
        var handler = new CapturingHandler((request, content) =>
        {
            Assert.Equal("/v1/audio/translations", request.RequestUri!.AbsolutePath);
            body = content;
            return Task.FromResult(JsonResponse("""{"text":"Tomorrow we discuss the changes.","language":"english"}"""));
        });
        using var client = new HttpClient(handler);
        var host = new TestPluginHostServices();
        host.Secrets["api-key"] = "sk-test";
        host.SetSetting("selectedModel", "whisper-1");
        var sut = new OpenAiPlugin(client, _ => new FakeTtsPlaybackSession(),
            _ => new OpenAiTranscriptionUpload([1, 2, 3], "audio.m4a", "audio/mp4"));
        await sut.ActivateAsync(host);
        var result = await sut.TranscribeAsync([4, 5, 6], "de", true, "Meeting planning", default);
        Assert.Equal("Tomorrow we discuss the changes.", result.Text);
        Assert.NotNull(body);
        Assert.DoesNotContain("name=language", body);
        Assert.DoesNotContain("name=\"language\"", body);
        Assert.Contains("Meeting planning", body);
        AssertMultipartToken(body, "filename", "audio.m4a");
    }
    ...
    public void ModelDiscovery_ExcludesNonChatEndpoints(string model) => Assert.False(OpenAiPlugin.IsChatModel(model));
    ...
    [InlineData("gpt-6-astra-2026-09-10")]
    public async Task ProcessAsync_UsesResponsesApiForModernReasoningModels(string model)
    {
        ...
        var result = await sut.ProcessAsync("Fix grammar", "hello world", model, CancellationToken.None);
    ...
    public async Task RefreshAvailableLlmModels_RefreshesExpiredChatGptTokenBeforeCatalogRequest()
    {
        ...
        Assert.Equal("new-access-token", JsonDocument.Parse(host.Secrets["chatgpt-session"]!).RootElement.GetProperty("AccessToken").GetString());
        Assert.Equal("new-refresh-token", JsonDocument.Parse(host.Secrets["chatgpt-session"]!).RootElement.GetProperty("RefreshToken").GetString());
    }
    ...
    public async Task ImportExistingLogin_LoadsTokensFromCodexAuthFile()
    {
        ...
        try
        {
            ...
            Assert.Equal("access-token", JsonDocument.Parse(host.Secrets["chatgpt-session"]!).RootElement.GetProperty("AccessToken").GetString());
            Assert.Equal("refresh-token", JsonDocument.Parse(host.Secrets["chatgpt-session"]!).RootElement.GetProperty("RefreshToken").GetString());
            Assert.Equal("acct_from_file", JsonDocument.Parse(host.Secrets["chatgpt-session"]!).RootElement.GetProperty("AccountId").GetString());
