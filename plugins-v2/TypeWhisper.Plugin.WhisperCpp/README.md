...
Version `1.2.13`; plugin ID `com.typewhisper.whisper-cpp`; minimum host `1.1.2`.
...
Source 1.2.13 verifies the persisted CUDA package identity and freshly computed DLL hashes on every check, stages complete files before atomic replacement and publishes its installation receipt last. Regression tests cover corruption, interrupted installation and package changes.
