...
Version `1.2.11`; plugin ID `com.typewhisper.whisper-cpp`; minimum host `1.1.2`.
...
Source 1.2.11 verifies the persisted CUDA package identity and freshly computed DLL hashes on every check, stages complete files before atomic replacement and publishes its installation receipt last. Regression tests cover corruption, interrupted installation and package changes.
...
Settings use a lightweight file-presence hint; model loading performs cancellable CUDA content verification on a worker thread. Download publication rejects empty or implausibly small model payloads and mismatched known stream lengths. This size check is not a cryptographic model-artifact guarantee.
