...
{
    private static readonly string Id="com.typewhisper.gladia";
    ...
    {
        var host=new Host(); using var plugin=new GladiaPlugin(); await plugin.ActivateAsync(host); await Configure(plugin);
    ...
    {
        var host=new Host(); using var plugin=new GladiaPlugin(); await plugin.ActivateAsync(host);
    ...
    public async Task ProtocolRoundTripPreservesUnicodeAndUsesExpectedEndpoint()
    {
        ...
        using var plugin=new GladiaPlugin(http); await plugin.ActivateAsync(new Host()); await Configure(plugin);
    ...
    {
        using var plugin=new GladiaPlugin();await plugin.ActivateAsync(new Host());await Configure(plugin);
    ...
    {
        var root=Path.Combine(Path.GetTempPath(),"gladia-package-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);
        try
        {
            ...
            var entry=new PortableCatalogEntry{Id=Id,Name="Gladia",Version="1.2.1",MinHostVersion="1.1.2",DownloadUrl="https://fixture.invalid/plugin.zip",Sha256=Convert.ToHexString(SHA256.HashData(bytes)),Size=bytes.Length,SupportedArchitectures=[PortablePluginCatalog.Architecture]};
            ...
            await using(var runtime=new PortablePluginRuntimeRegistry(store,new(1,1,2),_=>host))
            {
                ...
                Assert.Contains("de", Assert.Single(runtime.TranscriptionProviders).SupportedLanguages!);
