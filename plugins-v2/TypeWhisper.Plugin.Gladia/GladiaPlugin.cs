...
/// <summary>Independent portable Gladia provider, compared with Windows 4db8f6ac and macOS ac00e39e.</summary>
public sealed partial class GladiaPlugin : ITranscriptionEnginePlugin, IApiKeyPlugin, IPluginTextSettings
{
    private readonly ProviderConnection Connection;
    /// <summary>Creates a provider with isolated HTTP transport.</summary>
    public GladiaPlugin() : this(new HttpClient(new HttpClientHandler { AllowAutoRedirect = false }) { Timeout = TimeSpan.FromMinutes(5) }) { }
    internal GladiaPlugin(HttpClient http) => Connection = new(http);
    /// <inheritdoc />
    ...
    /// <inheritdoc />
    public IReadOnlyList<PluginModelInfo> TranscriptionModels { get; } = [new("default", "Gladia")];
    /// <inheritdoc />
