...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new GraniteSpeechPlugin();
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-GraniteSpeech-"+Guid.NewGuid().ToString("N"));
