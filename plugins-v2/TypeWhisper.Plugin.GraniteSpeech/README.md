...
Version `1.2.4`; plugin ID `com.typewhisper.granite-speech`; minimum host `1.1.2`.
...
After review fixes, 10 .NET tests and three Python tests pass. An isolated runtime upgrade to PyTorch 2.13.0 with TorchAudio 2.11.0 passed import, feature-extraction and pip dependency checks. Actual Granite inference produced the expected full English result and two-second preview on both CUDA and CPU. CPU-to-CUDA readiness, removal of the only selected model, dtype fallback and output-limit handling have regression coverage.
