...
public partial class OpenRouterPluginTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "openrouter-package-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            var source = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "bin", new DirectoryInfo(AppContext.BaseDirectory).Parent!.Name, "portable-host", "Plugins", "com.typewhisper.openrouter"));
            Assert.True(File.Exists(Path.Combine(source, "TypeWhisper.Plugin.OpenRouter.deps.json")));
            Assert.False(File.Exists(Path.Combine(source, "TypeWhisper.PluginSDK.dll")));
            var archive = Path.Combine(root, "openrouter.zip");
            ...
            {
                Id = "com.typewhisper.openrouter", Name = "OpenAI", Version = "1.1.0", MinHostVersion = "1.1.2",
            ...
            await using (var registry = new PortablePluginRuntimeRegistry(store, new(1, 1, 2), _ => host))
            {
                ...
                await registry.UseConfigurationAsync(entry.Id, async (plugin, ct) =>
                {
                    ...
                    await ((IPluginTextSettings)plugin).SaveTextSettingAsync("llmTemperatureMode", "custom", ct);
                    return true;
                });
            ...
            await using (var registry = new PortablePluginRuntimeRegistry(restart, new(1, 1, 2), _ => host))
            {
                ...
                Assert.Equal("custom", await registry.UseConfigurationAsync(entry.Id, (plugin, _) => Task.FromResult(((IPluginTextSettings)plugin).TextSettings.Single(s => s.Id == "llmTemperatureMode").Value)));
