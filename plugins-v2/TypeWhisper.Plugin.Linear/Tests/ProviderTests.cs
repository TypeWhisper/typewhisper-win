...
{
    private static readonly string Id="com.typewhisper.linear";
    ...
    {
        var host=new Host(); using var plugin=new LinearPlugin(); await plugin.ActivateAsync(host); await Configure(plugin);
    ...
    {
        var host=new Host(); using var plugin=new LinearPlugin(); await plugin.ActivateAsync(host);
    ...
    public async Task ProtocolRoundTripPreservesUnicodeAndUsesExpectedEndpoint()
    {
        ...
        using var plugin=new LinearPlugin(http); await plugin.ActivateAsync(new Host()); await Configure(plugin);
    ...
    {
        using var plugin=new LinearPlugin();await plugin.ActivateAsync(new Host());await Configure(plugin);
    ...
    {
        var root=Path.Combine(Path.GetTempPath(),"linear-package-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);
        try
        {
            ...
            var entry=new PortableCatalogEntry{Id=Id,Name="Linear",Version="1.1.0",MinHostVersion="1.1.2",DownloadUrl="https://fixture.invalid/plugin.zip",Sha256=Convert.ToHexString(SHA256.HashData(bytes)),Size=bytes.Length,SupportedArchitectures=[PortablePluginCatalog.Architecture]};
