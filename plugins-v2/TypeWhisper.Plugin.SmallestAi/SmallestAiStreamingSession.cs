...
internal sealed class SmallestAiStreamingSession : IStreamingSession
{
        ...
        CancellationToken ct, Uri? endpoint = null)
    {
        ...
        try { await ws.ConnectAsync(endpoint ?? BuildStreamingUri(language, wordTimestamps: true), ct); }
    ...
    public async Task FinalizeAsync(CancellationToken ct)
    {
        ...
        { await _lastResponseReceived.Task.WaitAsync(ct); return; }

        await _sendLock.WaitAsync(ct);
        try
        {
            if (_ws.State != WebSocketState.Open)
                throw new WebSocketException("The stream is no longer open.");
        ...
        try
        {
            while (!ct.IsCancellationRequested && _ws.State == WebSocketState.Open)
            {
                ...
                do
                {
                    ...
                    {
                        _lastResponseReceived.TrySetException(new WebSocketException("The provider closed before stream completion."));
                        return;
                    }
                    if (messageBuffer.Length + result.Count > 4 * 1024 * 1024) throw new InvalidOperationException("Streaming message exceeds the size limit.");
...
internal sealed class SmallestAiTranscriptCollector
{
    ...
    public StreamingTranscriptEvent? ApplyEvent(string json)
    {
        ...
        IsLastReceived |= GetBool(root, "is_last");
