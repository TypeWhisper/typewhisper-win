...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new SmallestAiPlugin();
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-SmallestAi-"+Guid.NewGuid().ToString("N"));
