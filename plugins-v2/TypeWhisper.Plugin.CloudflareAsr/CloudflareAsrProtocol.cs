...
public sealed partial class CloudflareAsrPlugin
{
    ...
    public async Task<PluginTranscriptionResult> TranscribeAsync(byte[] wavAudio,string? language,bool translate,string? prompt,CancellationToken ct)
    {
        ...
        return new(ProviderConnection.Text(result,"text") ?? throw ProviderConnection.InvalidResponse(),ProviderConnection.Text(result,"language"),ProviderConnection.Number(result,"duration"), NoSpeechProbability: null);
    }
    /// <inheritdoc />
