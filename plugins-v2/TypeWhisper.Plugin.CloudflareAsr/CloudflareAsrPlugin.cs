...
/// <summary>Independent portable Cloudflare Workers AI provider, compared with Windows 4db8f6ac and macOS ac00e39e.</summary>
public sealed partial class CloudflareAsrPlugin : ITranscriptionEnginePlugin, IApiKeyPlugin, IPluginTextSettings
{
    private readonly ProviderConnection Connection;
    /// <summary>Creates a provider with isolated HTTP transport.</summary>
    public CloudflareAsrPlugin() : this(new HttpClient(new HttpClientHandler { AllowAutoRedirect = false }) { Timeout = TimeSpan.FromMinutes(5) }) { }
    internal CloudflareAsrPlugin(HttpClient http) => Connection = new(http);
    /// <inheritdoc />
