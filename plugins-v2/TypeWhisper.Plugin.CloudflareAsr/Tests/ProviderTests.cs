...
{
    private static readonly string Id="com.typewhisper.cloudflare-asr";
    ...
    {
        var host=new Host(); using var plugin=new CloudflareAsrPlugin(); await plugin.ActivateAsync(host); await Configure(plugin);
    ...
    {
        var host=new Host(); using var plugin=new CloudflareAsrPlugin(); await plugin.ActivateAsync(host);
    ...
    public async Task ProtocolRoundTripPreservesUnicodeAndUsesExpectedEndpoint()
    {
        ...
        using var plugin=new CloudflareAsrPlugin(http); await plugin.ActivateAsync(new Host()); await Configure(plugin);
    ...
    {
        using var plugin=new CloudflareAsrPlugin();await plugin.ActivateAsync(new Host());await Configure(plugin);
    ...
    {
        var root=Path.Combine(Path.GetTempPath(),"cloudflare-asr-package-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);
        try
        {
            ...
            var entry=new PortableCatalogEntry{Id=Id,Name="CloudflareAsr",Version="1.1.10",MinHostVersion="1.1.2",DownloadUrl="https://fixture.invalid/plugin.zip",Sha256=Convert.ToHexString(SHA256.HashData(bytes)),Size=bytes.Length,SupportedArchitectures=[PortablePluginCatalog.Architecture]};
