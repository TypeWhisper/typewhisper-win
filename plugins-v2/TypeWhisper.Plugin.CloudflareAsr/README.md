...
Validation: 79 automated tests cover the callback, cancellation, PKCE, rejected state and duplicate parameters, credential persistence failures, concurrent refresh and account selection. On 2026-09-21, Marco confirmed browser consent and the automatic connection in version 1.1.10. A subsequent test explicitly verified OAuth mode, authenticated account/model access, and German Turbo transcription with the saved OAuth credentials. Automatic refresh is covered by fixtures, not a forced live token rotation. No live token values are included in fixtures or logs.
