// Protocol fixtures copied from the legacy provider tests at 3282e9ca; WPF UI fixtures excluded.
using System.IO;
using System.Net;
...
public partial class OpenAiCompatiblePluginTests
{
    ...
    public void Manifest_AdvertisesTranscriptionAndLlmCategories()
    {
        ...
        using var raw = JsonDocument.Parse(File.ReadAllText(Path.Combine(AppContext.BaseDirectory, "manifest.json")));
        Assert.Equal("transcription", raw.RootElement.GetProperty("category").GetString());
    ...
    public async Task AdditionalProfileRolesExposeIndependentSelectionIdsAndPersistSecretSeparately()
    {
        ...
        Assert.True(sut.Profiles.Single(item => item.Id == profile.Id).ThinkingEnabled);
