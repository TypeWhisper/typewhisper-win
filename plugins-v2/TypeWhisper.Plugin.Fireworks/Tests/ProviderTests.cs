...
{
    private static readonly string Id="com.typewhisper.fireworks";
    ...
    {
        var host=new Host(); using var plugin=new FireworksPlugin(); await plugin.ActivateAsync(host);
    ...
    {
        using var plugin=new FireworksPlugin();await plugin.ActivateAsync(new Host());await Configure(plugin);
    ...
    {
        var root=Path.Combine(Path.GetTempPath(),"fireworks-package-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);
        try
        {
            ...
            var entry=new PortableCatalogEntry{Id=Id,Name="Fireworks",Version="1.1.2",MinHostVersion="1.1.2",DownloadUrl="https://fixture.invalid/plugin.zip",Sha256=Convert.ToHexString(SHA256.HashData(bytes)),Size=bytes.Length,SupportedArchitectures=[PortablePluginCatalog.Architecture]};
