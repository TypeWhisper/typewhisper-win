...
{
    private sealed record ModelCatalog(PluginModelInfo[] Transcription, PluginModelInfo[] Text);
    private static readonly ModelCatalog DefaultCatalog = new(
        [new("voxtral-mini-latest", "Voxtral Mini Latest")],
        [new("mistral-small-latest", "Mistral Small Latest")]);
    ...
    public async Task<string?> ExecuteSettingsActionAsync(string id, CancellationToken cancellationToken)
    {
        ...
        foreach (var model in models.EnumerateArray())
        {
            ...
            // Realtime-only, OCR, embedding and speech generation models require other protocols.
