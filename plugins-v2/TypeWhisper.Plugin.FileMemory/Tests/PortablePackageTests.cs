...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new FileMemoryPlugin();
    ...
    public async Task Package_InstallsLoadsRestartsAndUninstallsIndependently()
    {
        ...
        await using (var runtime=new PortablePluginRuntimeRegistry(store,new(1,1,5),_ => fixture.Host))
        {
            ...
            var memory = Assert.Single(runtime.MemoryProviders);
            var action = Assert.Single(runtime.Actions);
            Assert.Equal(PortableActionStatus.Succeeded, (await runtime.ExecuteActionAsync(action, "Aurora uses British English", new(null,null,null,null,null))).Status);
            Assert.Equal("Aurora uses British English", Assert.Single(await runtime.SearchMemoryAsync(memory, "Write about Aurora", default)));
        ...
        await using (var runtime=new PortablePluginRuntimeRegistry(restart,new(1,1,5),_ => fixture.Host))
        {
            ...
            var previousMemory = Assert.Single(runtime.MemoryProviders);
            Assert.Null(await runtime.SetEnabledAsync(entry.Id,false));
            Assert.Empty(runtime.MemoryProviders);
            await Assert.ThrowsAsync<InvalidOperationException>(() => runtime.SearchMemoryAsync(previousMemory, "Aurora", default));
            Assert.Null(await runtime.SetEnabledAsync(entry.Id,true));
            await Assert.ThrowsAsync<InvalidOperationException>(() => runtime.SearchMemoryAsync(previousMemory, "Aurora", default));
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-FileMemory-"+Guid.NewGuid().ToString("N"));
