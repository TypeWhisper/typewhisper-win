...
{
    private static readonly string Id="com.typewhisper.speechmatics";
    ...
    {
        var host=new Host(); using var plugin=new SpeechmaticsPlugin(); await plugin.ActivateAsync(host); await Configure(plugin);
    ...
    {
        var host=new Host(); using var plugin=new SpeechmaticsPlugin(); await plugin.ActivateAsync(host);
    ...
    {
        var root=Path.Combine(Path.GetTempPath(),"speechmatics-package-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(root);
        try
        {
            ...
            var entry=new PortableCatalogEntry{Id=Id,Name="Speechmatics",Version="1.2.4",MinHostVersion="1.1.2",DownloadUrl="https://fixture.invalid/plugin.zip",Sha256=Convert.ToHexString(SHA256.HashData(bytes)),Size=bytes.Length,SupportedArchitectures=[PortablePluginCatalog.Architecture]};
            ...
            await using(var runtime=new PortablePluginRuntimeRegistry(store,new(1,1,2),_=>host))
            {
                ...
                await runtime.UseConfigurationAsync(Id,(p,_)=>{Assert.Equal(entry.Version,p.PluginVersion);Assert.Equal(p.PluginVersion,p.GetType().Assembly.GetName().Version!.ToString(3));return Task.FromResult(true);});
