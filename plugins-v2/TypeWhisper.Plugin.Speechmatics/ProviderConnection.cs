...
internal sealed class ProviderConnection(HttpClient http) : IDisposable
{
    ...
    internal static string? Language(string? language) => string.IsNullOrWhiteSpace(language) || language.Trim().Equals("auto", StringComparison.OrdinalIgnoreCase) ? null : language.Trim().ToLowerInvariant();
    internal static string[] Terms(string? prompt) => PluginDictionaryTerms.Clip(
        PluginDictionaryTerms.ParsePrompt(prompt), new(MaxTerms: 100, MaxTotalChars: 4000)).ToArray();
