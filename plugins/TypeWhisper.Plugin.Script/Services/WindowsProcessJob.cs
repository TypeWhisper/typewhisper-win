...
internal sealed class WindowsProcessJob : IDisposable
{
    ...
    public static WindowsProcessJob CreateAndAssign(Process process)
    {
        ...
        if (handle.IsInvalid)
        {
            ...
            handle.Dispose();
            throw new Win32Exception(error, "Failed to create the script process job.");
        }
        ...
        try
        {
            ...
            try
            {
                    ...
                    (uint)informationSize))
                {
                    throw new Win32Exception(
                        ...
                        "Failed to configure the script process job.");
                }
            }
            finally
            {
            ...
            if (!AssignProcessToJobObject(handle, process.Handle))
            {
                throw new Win32Exception(
                    Marshal.GetLastWin32Error(),
                    "Failed to attach script to the TypeWhisper process lifetime.");
            }
