    ...
    private static string? ParseEventStreamResponseText(string body)
    {
        ...
        foreach (var rawLine in body.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            switch (typeEl.GetString())
            {
                ...
                case "error":
                    throw new PluginRequestException("ChatGPT text processing failed.", PluginRequestFailureKind.ServerError);
        ...
        if (!responseCompleted) throw new PluginRequestException("ChatGPT disconnected before confirming the complete response.", PluginRequestFailureKind.Network);
