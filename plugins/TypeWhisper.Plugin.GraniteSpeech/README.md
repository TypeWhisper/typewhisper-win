...
Protocol/runtime sources and applicable fixtures were snapshotted from `plugins/TypeWhisper.Plugin.GraniteSpeech` at `4db8f6ac`, then adapted under `plugins`. The package does not reference the legacy provider DLL or compile WPF settings views. Legacy sources, catalogs and published packages remain unchanged.
...
```powershell
dotnet msbuild plugins/TypeWhisper.Plugin.GraniteSpeech/portable.proj /t:Build /p:Configuration=Release
dotnet test plugins/TypeWhisper.Plugin.GraniteSpeech/Tests -c Release
```
