...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new GemmaLocalPlugin();
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-GemmaLocal-"+Guid.NewGuid().ToString("N"));
