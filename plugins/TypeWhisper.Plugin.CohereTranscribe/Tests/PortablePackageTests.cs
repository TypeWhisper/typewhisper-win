...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new CohereTranscribePlugin();
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-CohereTranscribe-"+Guid.NewGuid().ToString("N"));
