...
Version `1.2.13`; plugin ID `com.typewhisper.meta`; minimum host `1.1.5`.
...
Protocol/runtime sources and applicable fixtures were snapshotted from `plugins/TypeWhisper.Plugin.Meta` at `4db8f6ac`, then adapted under `plugins`. The package does not reference the legacy provider DLL or compile WPF settings views. Legacy sources, catalogs and published packages remain unchanged.
...
```powershell
dotnet msbuild plugins/TypeWhisper.Plugin.Meta/portable.proj /t:Build /p:Configuration=Release
dotnet test plugins/TypeWhisper.Plugin.Meta/Tests -c Release
```
