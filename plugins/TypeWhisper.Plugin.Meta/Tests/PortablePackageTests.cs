...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new MetaPlugin();
    ...
    public async Task Package_InstallsLoadsRestartsAndUninstallsIndependently()
    {
        ...
        await Assert.ThrowsAsync<InvalidDataException>(() => PortablePluginPackage.LoadAsync(source, fixture.Host, new(1, 1, 4)));
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-Meta-"+Guid.NewGuid().ToString("N"));
