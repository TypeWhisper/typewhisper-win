...
public sealed class PortablePackageTests
{
    ...
    public async Task ActualHostServices_CanActivateAndReadAllSettingsWithoutWpf()
    {
        ...
        using ITypeWhisperPlugin plugin = new WhisperCppPlugin();
...
{
    internal string Root { get; } = Path.Combine(Path.GetTempPath(),"portable-WhisperCpp-"+Guid.NewGuid().ToString("N"));
