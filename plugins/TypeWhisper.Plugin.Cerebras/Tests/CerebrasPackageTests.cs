...
public partial class CerebrasTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "cerebras-package-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            var source = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "bin",
                new DirectoryInfo(AppContext.BaseDirectory).Parent!.Name, "portable-host", "Plugins", "com.typewhisper.cerebras"));
            Assert.Equal(new[] { "TypeWhisper.Plugin.Cerebras.deps.json", "TypeWhisper.Plugin.Cerebras.dll", "manifest.json" },
                Directory.GetFiles(source).Select(Path.GetFileName).Order(StringComparer.Ordinal));
            var archive = Path.Combine(root, "cerebras.zip");
            ...
            {
                Id = "com.typewhisper.cerebras", Name = "Cerebras", Version = "1.1.1", MinHostVersion = "1.1.2",
                DownloadUrl = "https://fixture.invalid/cerebras.zip", Size = bytes.Length,
            ...
            await using (var registry = new PortablePluginRuntimeRegistry(store, new(1, 1, 2), _ => host))
            {
                ...
                await registry.UseConfigurationAsync(entry.Id, async (plugin, ct) =>
                {
                    ...
                    await ((IPluginProfileSettings)plugin).SaveProfileSettingsAsync("cerebras", new Dictionary<string, string>
                    { ["llmTemperatureMode"] = "custom", ["selectedLlmModel"] = "qwen-3.8-27b" }, "fixture-key", ct);
                    return true;
                });
            ...
            await using (var registry = new PortablePluginRuntimeRegistry(restart, new(1, 1, 2), _ => host))
            {
                ...
                Assert.Equal("qwen-3.8-27b", await registry.UseConfigurationAsync(entry.Id, (plugin, _) =>
