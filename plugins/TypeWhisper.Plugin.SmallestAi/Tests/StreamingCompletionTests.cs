...
public sealed class StreamingCompletionTests
{
    ...
    public async Task Loopback_AwaitsTerminalResponseAndRejectsPrematureClose(bool prematureClose, bool subscriberThrows)
    {
        ...
        var server=Task.Run(async()=>
        {
            ...
            await Send(socket,"""{"type":"transcription","transcript":"Hallo","is_final":true}""",ct);
            var end=await Receive(socket,ct);Assert.Contains("close_stream",Encoding.UTF8.GetString(end));endReceived.TrySetResult();
            await release.Task.WaitAsync(ct);
            if(!prematureClose) { await Send(socket,"""{"type":"transcription","transcript":"Welt","is_final":true,"is_last":true}""",ct);  }
