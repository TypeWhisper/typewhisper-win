...
| CohereTranscribe | Independent v2 implementation in `plugins-v2/TypeWhisper.Plugin.CohereTranscribe`; 53 plugin tests pass. Shared device settings, brand icon, cold loading and local PCM preview supported. Native Q5_0/CUDA download, WAV inference and PCM preview verified; WinUI settings and live toggle verified; owner-confirmed microphone/live acceptance complete; publication pending. See the plugin README for platform differences. |
