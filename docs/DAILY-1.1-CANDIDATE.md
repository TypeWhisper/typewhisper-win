...
The internal `TypeWhisperDaily` identity is retained for existing installations. Both packages use the TypeWhisper display title. Each updater requires its installed package identity and rejects pre-1.1 packages. Stable and RC never fall back to the old WPF feeds. An exact owned startup command pointing at `TypeWhisper.WinUI.exe` is migrated to `TypeWhisper.exe`; unrelated commands are left untouched. An existing 1.0 Startup-folder shortcut remains in place without creating a second registration, and the startup settings can disable that owned shortcut.
...
Release profiles remain under `%LOCALAPPDATA%/TypeWhisper-WinUI`. Development profiles remain separate. Before opening profile stores, an absent release profile imports from the first of `%LOCALAPPDATA%/TypeWhisper-UserData` and `%LOCALAPPDATA%/TypeWhisper` that holds importable content: `settings.json`, `Data/` dictionary, snippets, workflows, history or license files, or a non-empty `PluginData/<id>` or `Plugins/<id>` folder. Empty folders left by an interrupted 1.0.4 migration (#318) are skipped. Existing 1.1 profiles are never merged or overwritten automatically.

Migration stages on the destination volume and publishes with a no-overwrite directory rename. It never writes to the source; even the backup export works on a private copy inside the staging directory. A symbolic link or junction at the legacy root, at its ancestors (for example a redirected `%LOCALAPPDATA%`) or at a configured external model location is resolved and read through; links inside the source are rejected. Leftover `.typewhisper-import-*` staging directories from a killed attempt are deleted at the start of the next import, under the single-instance lock.

Plugin catalog or package download failures, package validation failures, API keys that cannot be decrypted for the current Windows user and damaged per-plugin settings do not stop the migration: the profile is published and the affected plugins are listed in `legacy-migration-report.json` and the post-startup notice. Plugin settings and models are kept, so the plugins can be installed later in Integrations; undecryptable keys must be entered again. Linked paths inside the source, malformed core settings or profile data, and copy failures stop the migration without a visible partial profile. The failure window names the cause and offers **Retry** or **Start with a new profile**; the latter publishes an empty profile whose `legacy-import.json` records `skipped: true`, so the import is not retried, and leaves the previous profile untouched. Cancellation leaves no visible profile and can be retried. Close the previous app first.
