...
Research checkpoint: 2026-09-08. No HTTP listener has been added to WinUI by this work.
...
1. UI-independent host/router with mandatory token authentication except for minimal status, loopback-only binding, bounded uploads/concurrency, explicit enable/port controls, discovery lifecycle and safe shutdown.
2. GET /v1/status and GET /v1/models from real LocalDictationSession/provider state.
3. POST /v1/transcribe and POST /v1/transcribe/local-file using a shared request-aware transcription operation. Reject unsupported options explicitly; do not silently ignore model/language/prompt overrides.
4. Dictation session IDs and start/stop/result access, followed by recorder controller extraction and storage mutations.
5. Settings import must use the reviewed backup/restore drain transaction; it must not write stores behind active UI services.
