> Historical implementation journal, archived on 2026-09-23. Statements and test counts apply to the entries they accompany, not to the current release. Start with the [current documentation](../../README.md).
...
## Mac HTTP route parity and unchanged Raycast — 2026-09-08

- 140 targeted Presentation tests passed (HTTP transport, auth modes, parser, History, dictionary/rules, route catalog, dictation generation tracking, real RecorderController adapter and workflow/history regressions).
- 25 targeted portable plugin/model management tests passed, including unload cancellation/drain and preserving plugin enablement.
- Native `test_winui_http_api_parity.py`: all 29 Mac method/path pairs and wrong-method checks; token-free calls, History pagination/deletion, dictionary CRUD, profile toggle, Canary unload/load, recorder microphone capture → WAV → cleanup, and dictation session polling. Evidence: `artifacts/http-api-parity-native-evidence.json`.
- Changed portable backup imported with 202, concurrent import rejected with 409, app restarted and fixture was restored; own fixture removed afterwards. Reproducer: `tests/native/test_winui_http_api_backup.py`. Evidence: `artifacts/http-api-backup-native-evidence.json`.
- Authenticated lifecycle script passed again: required token, owner-only discovery, disable/corrupt settings cleanup, restart token persistence and real Canary transcription (7.69 s synthetic fixture), concurrent 200/409 and no History/source changes. Normal development profile restored via the prescribed helper.
- Marco confirmed that the existing Raycast extension works again. Raycast protocol action matches Mac (`raycast://extensions/SeoFood/typewhisper`), with Store fallback when no handler is installed. Final deep-link click is not automated here.

The parity profile disables AutoPaste, History saving and saved dictation audio during tests. Its model assets are copied only from the previous isolated local-model test fixture. Cloud inference, speech accuracy and actual deletion of installed model assets are not claimed by this run. Windows settings import uses the supported portable-profile format and asynchronous restart semantics; unsupported platform-specific backup content and unsupported transcription options return errors instead of being silently ignored.
