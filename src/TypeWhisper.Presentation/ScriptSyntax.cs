...
public static class ScriptSyntax
{
    private static readonly Regex PowerShell = new(
        """(?<Comment><#[\s\S]*?(?:#>|\z)|\#[^\r\n]*)|(?<String>@'\r?\n[\s\S]*?(?:\r?\n'@|\z)|@"\r?\n[\s\S]*?(?:\r?\n"@|\z)|'(?:''|[^'])*(?:'|\z)|"(?:`.|[^"`])*(?:"|\z))|(?<Variable>\$(?:\{[^}]*\}|[\w:?]+))|(?<Type>\[[\w.\[\],]+\])|(?<Keyword>\b(?:if|else|elseif|foreach|for|while|do|switch|return|function|param|try|catch|finally|throw|in|begin|process|end)\b|\b[A-Za-z]+-[A-Za-z]+\b|-(?:match|replace|split|join|notmatch|eq|ne|gt|lt|and|or|not)\b)|(?<Number>\b\d+(?:\.\d+)?\b)""",
