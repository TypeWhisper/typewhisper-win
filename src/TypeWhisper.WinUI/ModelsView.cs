...
public sealed class ModelsView : UserControl
{
    ...
    private sealed record ModelRow(ModelOption Model, Border Card, TextBlock Status,
        HandCursorButton Action, HandCursorButton Cancel, Border Progress, Border Fill);

    internal ModelsView(Dictionary<string, string> values)
    {
        _session = new(values);
        Tag = "SelectedModelId";
        ...
        foreach (var model in ModelSession.Models) panel.Children.Add(CreateRow(model));
    ...
    private FrameworkElement CreateRow(ModelOption model)
    {
    ...
    private void Update()
    {
        var active = ModelSession.Models.FirstOrDefault(model => model.Id == _session.Active);
        ...
        foreach (var row in _rows)
        {
            ...
            row.Action.Style = (Style)Application.Current.Resources[downloaded ? "PrimaryButtonStyle" : "SecondaryButtonStyle"];
