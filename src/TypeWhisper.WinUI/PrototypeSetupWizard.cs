...
public sealed class PrototypeSetupWizard : UserControl
{
        ...
        switch (_state.Step)
        {
            ...
            case 2:
                ...
                _body.Children.Add(new PrototypeShortcutRecorder("MainDictationHotkeys", "Main dictation", "Ctrl+Shift+F9", _values,
                    () => PrototypeSettingsCatalog.ShortcutBindings(_values), value => _closing ? "Setup is closed." : _commitHotkeys(value)));
    ...
    private void RefreshModelPickers()
    {
        ...
        try
        {
            ...
            _providerPicker.SetOptions(providers.Select(item => new PrototypeChoice(item.Id, item.Name, item.Status + (item.Cloud ? " - cloud" : " - on device"))).ToArray(), _selectedProvider ?? "", "Provider unavailable");
            _modelPicker.SetOptions(selected?.Models.Where(item => item.Ready).Select(item => new PrototypeChoice(item.Id, item.Name, "Ready")).ToArray() ?? [],
            ...
            _languagePicker.SetOptions(options.Select(code => new PrototypeChoice(code, LanguageName(code), "Supported by the active model")).ToArray(), _session.Language);
    ...
    {
        var button = new HandCursorButton { Content = title, Style = (Style)Application.Current.Resources["PrototypeSecondaryButtonStyle"] };
