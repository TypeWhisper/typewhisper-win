...
public sealed class TargetAppCorrectionLearningService
{
    ...
    private readonly Func<IReadOnlyList<CorrectionSuggestion>, IReadOnlyList<LearnedDictionaryCorrection>> _learn;
    ...
    public TargetAppCorrectionLearningService(
        Func<IReadOnlyList<CorrectionSuggestion>, IReadOnlyList<LearnedDictionaryCorrection>> learn,
    ...
    {
        var learned = _learn(suggestions);
        ...
        string currentValue)
    {
        ...
        var originalWords = insertedText.Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries);
        var editedWords = editedInsertedText.Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries);
        if (originalWords.Length != editedWords.Length) return new(TargetAppCorrectionLearningOutcomeKind.AmbiguousEdit, []);
        var changedWords = originalWords.Zip(editedWords).Count(pair => !string.Equals(pair.First, pair.Second, StringComparison.OrdinalIgnoreCase));
        if (changedWords > 3 || (originalWords.Length > 1 && changedWords * 2 > originalWords.Length))
            return new(TargetAppCorrectionLearningOutcomeKind.AmbiguousEdit, []);
