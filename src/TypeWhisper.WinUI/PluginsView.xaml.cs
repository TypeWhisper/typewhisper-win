...
public sealed partial class PluginsView : UserControl
{
    ...
    internal void InstallSample(Plugin plugin)
    {
    ...
    internal void Filter(string query)
    {
        if (IsDetail) return;
        _query = query;
        var selectedId = (PluginList.SelectedItem as Plugin)?.Id ?? _opened?.Id;
        ...
        foreach (var plugin in _plugins.Where(plugin =>
            ...
            && (plugin.Title.Contains(query, StringComparison.OrdinalIgnoreCase) || plugin.Categories.Contains(query, StringComparison.OrdinalIgnoreCase))))
    ...
    {
        if (IsDetail || PluginList.SelectedItem is not Plugin plugin) return;
        _opened = plugin;
        ShowPage(Page.Detail);
        PluginDetailPage.ChangeView(null, 0, null, true);
        FocusDetailAction();
    ...
    private void ShowPage(Page page)
    {
        ...
        if (page == Page.Detail && _opened is { } plugin)
        {
            ...
            PluginStatusExplanation.Text = !plugin.Compatible ? $"Requires TypeWhisper {plugin.MinimumHostVersion} or later. This preview represents host 1.1; this example cannot be enabled."
                : !plugin.Enabled ? "Disabled in this preview. Your example settings are kept."
    ...
    private void Primary_Click(object sender, RoutedEventArgs e)
    {
        ...
        var export = _opened.Categories.StartsWith("Export", StringComparison.Ordinal);
    ...
    private void Replace(Plugin updated)
    {
