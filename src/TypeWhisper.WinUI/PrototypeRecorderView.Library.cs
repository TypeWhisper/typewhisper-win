...
public sealed partial class PrototypeRecorderView
{
    ...
    {
        var row = new StackPanel { Spacing = 6, Margin = new Thickness(0, 0, 0, 14) };
        ...
        row.Children.Add(new TextBlock { Text = $"{duration} · {entry.SizeBytes:N0} bytes · {date}", FontSize = 12, TextWrapping = TextWrapping.Wrap });
    ...
    private static void AddLibraryAction(Panel panel, string text, Action action, bool enabled = true)
    {
        var button = new HandCursorButton { Content = text, IsEnabled = enabled };
