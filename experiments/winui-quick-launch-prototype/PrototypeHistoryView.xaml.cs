...
{
    private readonly PrototypeHistoryStore _store = new(PrototypeHistorySamples.Entries.Select(item => item.Entry));
    private readonly Dictionary<Guid, string> _timeLabels = PrototypeHistorySamples.Entries.ToDictionary(item => item.Entry.RecordId, item => item.Time);
    ...
    private void Devices_Opening(object sender, object e)
    {
        ...
        foreach (var device in _store.Devices)
            AddDeviceChoice(device.DeviceId, device.DeviceName ?? device.Platform,
                $"{device.Platform} · {(device.DeviceId == PrototypeHistoryDevices.ThisPc.DeviceId ? "This device" : "Sample device")}", DeviceIcon(device.Platform));
    }
