...
{
    private readonly IReadOnlyList<MarketplaceItem> _catalog = MarketplaceItem.Samples;
    private Func<string, bool> _isInstalled = _ => false;
    private Action<Plugin>? _install;
    ...
    internal void ConfigureInventory(Func<string, bool> isInstalled, Action<Plugin> install)
    {
    ...
    {
        if (IsDetail || MarketList.SelectedItem is not MarketplaceItem item) return;
        _opened = item;
        IsDetail = true;
    ...
    private void UpdateDetail(bool updateBreadcrumbs = true)
    {
        ...
        MarketStatusExplanation.Text = busy ? "No download is running. Cancel or go back to stop this preview."
            ...
            : !item.Plugin.Compatible ? $"This example requires TypeWhisper {item.Plugin.MinimumHostVersion} or later. The preview represents host 1.1."
