...
public sealed class ActivityView : UserControl
{
    ...
    private void Render()
    {
        ...
        var data = new UsageData(_empty);
        var summary = _statistics && _period == UsagePeriod.Custom
            ? UsageData.SummarizeRange(data.Events, _rangeStart, _rangeEnd)
        ...
        if (_statistics)
        {
            ...
            {
                var label = period switch { UsagePeriod.Week => "Week", UsagePeriod.Month => "Month", _ => "All time" };
    ...
    private void RenderDashboard(UsageSummary summary)
    {
        ...
        foreach (var item in HistorySamples.Entries.Take(3))
        {
            ...
            if (item != HistorySamples.Entries[2]) recent.Children.Add(new Border { Height = 1, Background = Brush("HairlineBrush") });
        }
