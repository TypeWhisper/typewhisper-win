...
internal enum FileStatus { Queued, Processing, Ready, Canceled, Failed }
internal sealed class FileJob(string path, bool fail = false)
{
    internal Guid Id { get; } = Guid.NewGuid();
    internal string Path { get; } = path;
    internal string Name => System.IO.Path.GetFileName(Path);
    internal bool SimulateFailure { get; } = fail;
    internal FileStatus Status { get; set; }
...
internal sealed class FileQueue
{
    ...
    internal List<FileJob> Jobs { get; } = [];
    ...
    internal bool Start()
    {
        if (Running || !Jobs.Any(job => job.Status == FileStatus.Queued)) return false;
        Running = true; return true;
    }
    internal void Tick()
    {
        if (!Running) return;
        var job = Jobs.FirstOrDefault(job => job.Status == FileStatus.Processing)
            ?? Jobs.FirstOrDefault(job => job.Status == FileStatus.Queued);
        if (job is null) { Running = false; return; }
        job.Status = FileStatus.Processing; job.Progress = Math.Min(100, job.Progress + 10);
        if (job.SimulateFailure && job.Progress >= 50) job.Status = FileStatus.Failed;
        else if (job.Progress == 100) job.Status = FileStatus.Ready;
        Running = Jobs.Any(item => item.Status is FileStatus.Queued or FileStatus.Processing);
    }
    internal void Cancel()
    {
        Running = false;
        foreach (var job in Jobs.Where(job => job.Status is FileStatus.Queued or FileStatus.Processing)) job.Status = FileStatus.Canceled;
    }
    internal bool Retry(FileJob job)
    {
        if (Running || !Jobs.Contains(job) || job.Status is not (FileStatus.Canceled or FileStatus.Failed)) return false;
        job.Status = FileStatus.Queued; job.Progress = 0; return true;
    }
    internal bool Remove(FileJob job) => !Running && Jobs.Remove(job);
    internal static string Export(FileJob job, string format)
    {
        if (job.Status != FileStatus.Ready) throw new InvalidOperationException("The result is not ready.");
