# Application expert walkthrough — 2026-09-05
...
- Moderate / high confidence: backup review offered only cancel/confirm, with no way to revise the chosen categories. Added Back preserving the selection (`SyncBackupView.cs`). Data retention verified in code; full Back interaction still pending.
- Moderate / high confidence: settings-level Escape previously closed the window while a backup preview was open. Added preview-first dismissal and deferred focus transfer (`SyncBackupView.cs`, `SettingsWindow.xaml.cs`); retested live.
