...
Run: build `TypeWhisper.WinUI.csproj -c Debug -r win-x64`, then launch its isolated executable with `--sync-backup`. Also available under Settings > Sync & backup.

Artifact: `SyncBackupView.cs`. Integration changes: `SettingsCatalog.cs`, `SettingsWindow.xaml.cs`, `MainWindow.xaml.cs`, `App.xaml.cs`.

References inspected: Mac `HistorySettingsView.swift`, `BackupRestoreSheets.swift`, and `SettingsBackupExporter.swift`. The preview represents a subset of backup categories, not a compatible serialization contract. Audio and secrets are excluded in the UI. Production migration still needs schema/platform mapping, conflict semantics and actual service wiring.
