...
public sealed class DateRangePicker : UserControl
{
    ...
    private void Apply()
    {
        if (!Parse(_from.Text, out var start) || !Parse(_to.Text, out var end) || UsageData.ValidateRange(start, end) is not null) return;
    ...
    {
        var button = new HandCursorButton { Content = text, Style = (Style)Application.Current.Resources[primary ? "PrimaryButtonStyle" : "SecondaryButtonStyle"] };
