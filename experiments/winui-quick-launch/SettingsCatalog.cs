...
internal static partial class SettingsCatalog
{
    ...
    [
        ...
        Text("Shortcuts", "PushToTalkHotkey", "Push to talk", "Ctrl+Shift"),
        ...
        Text("Models", "LocalModelStoragePath", "Model storage folder", "", "Empty uses the default folder. No files are moved in this preview."),
    ...
    internal static void Render(string category, StackPanel target, Dictionary<string, string> values, List<ChoicePicker> pickers, Action? refresh = null)
    {
        ...
        if (category == "Models")
        {
            ...
            target.Children.Add(new ModelsView(values));
        ...
        if (category == "Shortcuts")
        {
            ...
            ToolTipService.SetToolTip(guide, "Preview session only. Conflicts are checked within TypeWhisper, not against Windows or other apps. All actions support modifier combinations such as Shift + Win; global availability is not tested here.");
                ...
                foreach (var key in group.Keys)
                {
                    ...
                    item.Children.Add(new ShortcutRecorder(field.Key, field.Label, field.Value, values,
                        () => Fields.Where(f => f.Category == "Shortcuts").Select(f =>
                            (f.Key, f.Label, values.GetValueOrDefault(f.Key, f.Value)))));
            ...
            target.Children.Add(Label("Local preview · no global shortcuts are registered.", 12, true));
            return;
        }
