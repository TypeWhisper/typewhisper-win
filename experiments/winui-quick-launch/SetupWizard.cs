...
// Mac-inspired setup flow. Only the preview's session dictionary is shared.
public sealed class SetupWizard : UserControl
{
    ...
    private void Render()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            stack.Children.Add(Copy(SetupState.Steps[i], 12, i != _state.Step));
            var step = Button("", () => { _state.Revisit(index); Render(); });
            step.Content = stack; step.Style = (Style)Application.Current.Resources["IconButtonStyle"];
            step.HorizontalAlignment = HorizontalAlignment.Stretch;
            AutomationProperties.SetName(step, $"Step {i + 1} of 5: {SetupState.Steps[i]}");
            ...
            if (i >= _state.Step) AutomationProperties.SetName(item, $"Step {i + 1} of 5: {SetupState.Steps[i]}, {(i == _state.Step ? "current" : "upcoming")}");
        ...
        switch (_state.Step)
        {
            ...
            case 2:
                ...
                _body.Children.Add(new ShortcutRecorder("MainDictationHotkeys", "Main dictation", "Ctrl+Shift+F9", _values,
                    () => SettingsCatalog.ShortcutBindings(_values)));
                ...
                break;
            case 3:
                var session = new ModelSession(_values);
                var picker = new ChoicePicker(); picker.Configure("Model", "chip", "Setup model");
                picker.SetOptions(ModelSession.Models.Where(m => session.IsDownloaded(m.Id))
                    .Select(m => new Choice(m.Id, m.Title, m.Description)).ToArray(), session.Active);
                ...
                break;
            case 4:
                var model = ModelSession.Models.FirstOrDefault(m => m.Id == new ModelSession(_values).Active)?.Title ?? "Not selected";
    ...
    {
        _body.Children.Add(new SetupLogo { HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 12, 0, 12) });
    }
    private void AddPicker(string label, string icon, string key, string fallback, string[] options, string[]? labels = null)
    {
        ...
        var picker = new ChoicePicker(); picker.Configure(label, icon, $"Setup {label}");
        picker.SetOptions(options.Select((value, i) => new Choice(value, labels?[i] ?? value, "Session-only preview")).ToArray(), _values.GetValueOrDefault(key, fallback));
    ...
    {
        var button = new HandCursorButton { Content = title, MinHeight = 40, Style = (Style)Application.Current.Resources[primary ? "PrimaryButtonStyle" : "SecondaryButtonStyle"] };
