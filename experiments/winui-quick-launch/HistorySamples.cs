...
internal static class HistorySamples
{
    internal static readonly Transcript[] Entries =
    [
    ...
    private static Transcript Sample(int number, string title, string timeLabel,
        double seconds, string language, string text)
    {
        ...
        {
            3 or 6 => HistoryDevices.Phone,
            5 => HistoryDevices.Mac,
            _ => HistoryDevices.ThisPc
        };
        ...
        var entry = new HistoryEntry(
            ...
            new HistoryContent(createdAt, createdAt, origin, source,
                number == 5 ? HistoryEntryKind.Recording : HistoryEntryKind.Dictation,
                title, seconds, HistoryProcessingState.Ready,
                new HistoryTranscript(text, text), language),
            new HistoryInbox(createdAt),
            number == 5 ? new HistoryAudio(createdAt, "audio/mp4", 1234,
                "assets/history/preview-mac-recording/audio.m4a", new string('a', 64)) : null)
        {
            LocalState = new HistoryLocalState(IsSample: true)
        };
        entry.Validate();
        return new Transcript(entry, timeLabel);
    }
}
