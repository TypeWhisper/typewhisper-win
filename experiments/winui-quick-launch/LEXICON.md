...
Launch its `bin/Debug/net10.0-windows10.0.26100.0/win-x64/TypeWhisper.WinUI.exe` with `--dictionary` or `--snippets`. Close the running preview before building or changing startup flags. Do not launch the installed production app.
...
- `Lexicon.cs`: immutable entry records and session store, independent of WinUI.
- `LexiconView.cs`: shared list/editor UI and draft navigation.
...
- `../winui-lexicon-check`: standalone model checks.

## Verification

```powershell
dotnet run --project experiments/winui-lexicon-check/TypeWhisper.LexiconCheck.csproj
```
