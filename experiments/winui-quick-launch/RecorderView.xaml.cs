...
public sealed partial class RecorderView : UserControl
{
    ...
    internal HistoryEntry? CompletedEntry { get; private set; }
    ...
    internal event Action<HistoryEntry>? CompletedEntryChanged;
    ...
    private void Primary_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (_state is SessionState.Recording or SessionState.Paused)
        {
            ...
            CompletedEntry = HistoryEntry.CreateRecorderSample(_sessionId, _sessionStartedAt,
                completedAt >= _sessionStartedAt ? completedAt : _sessionStartedAt,
                EffectiveTitle, _elapsed.Elapsed.TotalSeconds,
                (_usedMicrophone ? CaptureInputs.Microphone : CaptureInputs.None)
                | (_usedSystemAudio ? CaptureInputs.SystemAudio : CaptureInputs.None));
            CompletedEntryChanged?.Invoke(CompletedEntry);
        }
