...
internal sealed class SetupState(Dictionary<string, string> values)
{
    internal static readonly string[] Steps = ["Welcome", "Microphone", "Shortcut", "Model", "Try it"];
    internal int Step { get; private set; } = Math.Clamp(int.TryParse(values.GetValueOrDefault("Setup.Step"), out var saved) ? saved : 0, 0, 4);
    internal string? Validation => Step switch
    {
        ...
        3 when !ModelSession.Models.Any(model => model.Id == new ModelSession(values).Active
            && new ModelSession(values).IsDownloaded(model.Id)) => "Choose a downloaded sample model before continuing, or skip setup.",
        _ => null
    };
    internal bool Next()
    {
    ...
    private void MoveTo(int step) { Step = step; values["Setup.Step"] = step.ToString(); }
}
