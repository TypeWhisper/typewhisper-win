...
internal sealed class SyncBackupView : UserControl
{
    ...
    internal SyncBackupView(Dictionary<string, string> values)
    {
        ...
        var toggle = AppToggleSwitch.Create(On("Enabled"));
        ...
        _body.Children.Add(Copy("Application only · no account connection, file export or restoration. Device and backup contents are fictional.", 12, true));
    }
    ...
    private void ShowSelection(bool restore, string[]? previous = null)
    {
        ...
        {
            var toggle = AppToggleSwitch.Create(selected.Contains(category));
    ...
    {
        var toggle = AppToggleSwitch.Create(On(key));
