...
public sealed class FileTranscriptionView : UserControl
{
    ...
    private void Render()
    {
        ...
        errorDemo.Style = (Style)Application.Current.Resources["IconButtonStyle"]; errorDemo.IsEnabled = !_queue.Running && !_picking; _body.Children.Add(errorDemo);
        ...
        else
        {
            ...
            start.IsEnabled = !_picking && _queue.Jobs.Any(job => job.Status == FileStatus.Queued); _actions.Children.Add(start);
        }
    }
    private void AddRow(FileJob job)
    {
        ...
        void Update() { status.Text = job.Status == FileStatus.Failed ? "Demo error · simulated unreadable file" : job.Status == FileStatus.Processing ? $"Simulating · {job.Progress}%" : job.Status == FileStatus.Ready ? "Ready · sample transcript" : job.Status.ToString(); fill.Width = track.ActualWidth * job.Progress / 100; }
        ...
        if (job.Status == FileStatus.Ready) actions.Children.Add(Button("View result", () => { _result = job; _scroll.ChangeView(null, 0, null, true); Render(); }));
        if (job.Status is FileStatus.Failed or FileStatus.Canceled)
        {
    ...
    private void RenderResult(FileJob job)
    {
    ...
    private async Task ChooseFiles()
    {
        ...
        try
        {
            ...
            foreach (var extension in FileQueue.Extensions) picker.FileTypeFilter.Add(extension);
    ...
    private async Task Export(FileJob job)
    {
        ...
        try
        {
            ...
            await writer.WriteAsync(FileQueue.Export(job, format));
