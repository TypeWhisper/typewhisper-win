...
{
    private readonly HistoryStore _store = new(HistorySamples.Entries.Select(item => item.Entry));
    private readonly Dictionary<Guid, string> _timeLabels = HistorySamples.Entries.ToDictionary(item => item.Entry.RecordId, item => item.Time);
    ...
    private void ApplyFilters()
    {
        ...
        DictationFilter.Style = FilterStyle(_kind == HistoryEntryKind.Dictation);
        RecordingFilter.Style = FilterStyle(_kind == HistoryEntryKind.Recording);
    ...
    {
        _kind = Enum.TryParse<HistoryEntryKind>((string)((Button)sender).Tag, out var kind) ? kind : null;
        ApplyFilters();
    }
    ...
    private void Devices_Opening(object sender, object e)
    {
        ...
        foreach (var device in _store.Devices)
            AddDeviceChoice(device.DeviceId, device.DeviceName ?? device.Platform,
                $"{device.Platform} · {(device.DeviceId == HistoryDevices.ThisPc.DeviceId ? "This device" : "Sample device")}", DeviceIcon(device.Platform));
    }
    ...
    internal void OpenSelected()
    {
        ...
        ResultSummary.Text = "Sample entry · not synced";
    ...
    private void UpdateResultSummary() => ResultSummary.Text =
        $"{FilteredEntries.Count} {(FilteredEntries.Count == 1 ? "entry" : "entries")} · sample data only";
