...
public sealed partial class WorkflowsView : UserControl
{
    ...
    private readonly List<WorkflowDraft> _workflows = WorkflowDraft.Samples.ToList();
    ...
    private static readonly Choice[] Providers = [new("none", "Not configured", "Set up a provider later"),
        new("local", "Local engine", "Demo provider · no model loaded"), new("cloud", "Cloud provider", "Demo provider · no account connected")];
    private static readonly Choice[] LocalModels = [new("local-small", "Small demo model", "Example model choice"), new("local-large", "Large demo model", "Example model choice")];
    private static readonly Choice[] CloudModels = [new("cloud-fast", "Fast demo model", "Example model choice"), new("cloud-quality", "Quality demo model", "Example model choice")];
    private static readonly Choice[] Outputs = [new("preview", "Preview in Quick Launch", "Review the result before using it"),
    ...
    private void ShowPage(Page page)
    {
        ...
        WorkflowExecutionSummary.Text = _opened is null || _opened.ProviderId == "none"
            ? "No provider connected · nothing leaves this preview"
    ...
    private void UpdateSourceState()
    {
        ...
        WorkflowInputHint.Text = _opened is { HasExample: false } ? "Custom workflow · execution is not connected in this preview."
    ...
    private IReadOnlyList<Choice> Models => ConfigProvider.SelectedId switch { "local" => LocalModels, "cloud" => CloudModels, _ => Array.Empty<Choice>() };
    ...
    private void NewWorkflow_Click(object sender, RoutedEventArgs e)
    {
        ...
        _creating = true;
        _opened = new WorkflowDraft(Guid.NewGuid().ToString("N"), "", "Custom workflow · this session only", "workflow", "", "", "");
    ...
    private void UpdateConfigurationState()
    {
        ...
        ConfigurationValidation.Text = error ?? "Saved only for this preview session.";
        ...
        WorkflowSummary.Text = ConfigurationDirty ? "Unsaved changes" : "Application configuration";
