...
internal enum UsagePeriod { Week, Month, AllTime, Custom }
internal sealed record UsageEvent(DateTime At, int Words, double Minutes, string App, string Model);
internal sealed record UsageDay(DateOnly Date, int Words);
internal sealed record UsageRank(string Name, int Count);
...
internal sealed class UsageData
{
    ...
    internal IReadOnlyList<UsageEvent> Events { get; }
    internal UsageData(bool empty = false)
    {
    ...
    internal UsageSummary Summarize(UsagePeriod period) => Summarize(Events, Today, period);
    internal static UsageSummary Summarize(IEnumerable<UsageEvent> source, DateOnly today, UsagePeriod period)
    {
        var all = source.Where(item => DateOnly.FromDateTime(item.At) <= today).ToArray();
        var start = period switch { UsagePeriod.Week => today.AddDays(-6), UsagePeriod.Month => today.AddDays(-29), _ => all.Length == 0 ? today : all.Min(item => DateOnly.FromDateTime(item.At)) };
    ...
    internal static UsageSummary SummarizeRange(IEnumerable<UsageEvent> source, DateOnly start, DateOnly end)
    {
        ...
        var days = Enumerable.Range(0, end.DayNumber - start.DayNumber + 1).Select(offset => start.AddDays(offset))
            .Select(date => new UsageDay(date, events.Where(item => DateOnly.FromDateTime(item.At) == date).Sum(item => item.Words))).ToArray();
        UsageRank[] Rank(Func<UsageEvent, string> key) => events.GroupBy(key).Select(group => new UsageRank(group.Key, group.Count())).OrderByDescending(item => item.Count).ThenBy(item => item.Name).ToArray();
...
internal sealed record UsageSummary(int Words, double Minutes, int Transcriptions, int ActiveDays,
    int CurrentStreak, int LongestStreak, UsageDay[] Days, UsageRank[] Apps, UsageRank[] Models, int[,] Hours)
{
    internal int Wpm => Minutes > 0 ? (int)(Words / Minutes) : 0;
    // Explicit preview estimate; not a claim about measured personal productivity.
