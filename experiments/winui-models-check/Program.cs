...
var session = new ModelSession(values);
...
var reopened = new ModelSession(values);
...
var setup = new SetupState(setupValues);
...
new ModelSession(setupValues).Activate("demo-balanced");
Check(setup.Next() && setup.Step == 4, "Downloaded active model permits trial");
Check(!setup.Next(), "Final step stays within bounds");
Check(new SetupState(setupValues).Step == 4, "Setup resumes within session");
...
setupValues["Setup.Step"] = "900";
Check(new SetupState(setupValues).Step == 4, "Invalid saved step clamped");
